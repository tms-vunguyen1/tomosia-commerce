...
``start`` is the only place a principal enters the store: it binds the operator to a
fresh unguessable session id. Every later request carries that id in ``SESSION_HEADER``
and the routes read the principal from the record, so no request shape names a user.

A session is stored as two things: a small state document (provenance state, the
operator binding, queued app events), rewritten under a new version only when it
