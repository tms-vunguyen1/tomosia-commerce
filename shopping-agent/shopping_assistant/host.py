...
def load_env() -> None:
    ...
    wins; ``shopping-agent/.env`` fills in the rest."""
...
def require_internal_token(token: str | None = Header(default=None, alias=INTERNAL_HEADER)) -> None:
    ...
    if token != require_env("ASSISTANT_INTERNAL_TOKEN"):
...
def _lifespan(
    ...
    async def lifespan(_: FastAPI) -> AsyncIterator[None]:
        if not (os.environ.get("ANTHROPIC_API_KEY") or os.environ.get("ANTHROPIC_AUTH_TOKEN")):
            logger.info(
                "No API key in the environment or shopping-agent/.env; the Anthropic SDK "
                "falls back to its own credential chain. If chat returns auth errors, set "
                "ANTHROPIC_API_KEY in shopping-agent/.env."
            )
...
def build_app(
    ...
    (``ASSISTANT_ALLOWED_HOSTS``, loopback and the compose service name by default).
    Rejecting other Host headers stops DNS-rebinding, which CORS does not; the browser
    never reaches this service directly, so there is no CORS policy to relax."""
    logging.basicConfig(
        level=os.environ.get("ASSISTANT_LOG_LEVEL", "INFO").upper(),
        format="%(levelname)s %(name)s: %(message)s",
    )
    logging.getLogger("httpx").setLevel(logging.WARNING)
    configured = os.environ.get("ASSISTANT_ALLOWED_HOSTS", "localhost,127.0.0.1,api")
...
# English fallback for a client with no copy of src/lib/assistant/errors.ts yet.
_CHAT_ERROR_TEXT = {
    errors.CHAT_AUTH_FAILED: (
        "Anthropic API authentication failed. Check ANTHROPIC_API_KEY in "
        "shopping-agent/.env and restart the service."
    ),
