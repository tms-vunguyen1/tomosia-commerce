// No "use server" here on purpose: it would turn every export in this file —
// including the customer and cart calls — into an endpoint the browser can
// invoke. Client components import the narrow surface in ./actions instead.
import {
  HIDDEN_PRODUCT_TAG,
...
import { getCartDeliveryGroupsQuery, getCartQuery } from "./queries/cart";
...
import {
  ...
  ShopifyCart,
  ShopifyCartDeliveryAddressesAddOperation,
  ShopifyCartDeliveryGroupsOperation,
...
/** Sets the cart's delivery address, so the next `getCartDeliveryOptions` quotes
 * against it. Throws on a Shopify user error (an incomplete or unrecognized address). */
export async function addCartDeliveryAddress(
  cartId: string,
  address: CartDeliveryAddressInput,
): Promise<void> {
  const res = await shopifyFetch<ShopifyCartDeliveryAddressesAddOperation>({
    query: cartDeliveryAddressesAddMutation,
    variables: { cartId, address },
    cache: "no-store",
  });
  const errors = res.body.data.cartDeliveryAddressesAdd.userErrors;
  if (errors.length) {
    throw new Error(errors.map((e) => e.message).join("; "));
  }
}

/** The delivery options for a cart that already has an address
 * (`addCartDeliveryAddress`); flattened across delivery groups, since this store's
 * carts ship as one group. */
export async function getCartDeliveryOptions(
  cartId: string,
): Promise<ShopifyDeliveryOption[]> {
  const res = await shopifyFetch<ShopifyCartDeliveryGroupsOperation>({
    query: getCartDeliveryGroupsQuery,
    variables: { cartId },
    cache: "no-store",
  });
  const groups = res.body.data.cart ? removeEdgesAndNodes(res.body.data.cart.deliveryGroups) : [];
  return groups.flatMap((group) => group.deliveryOptions);
}
...
/**
 * A product or a variant by its global id, for callers that hold an id rather than a
 * handle (the shopping assistant's internal API). A variant comes back as its parent
 * product plus the variant the id named, so the caller can tell the two apart.
 */
export async function getProductById(
  id: string,
): Promise<{ product: Product; variantId?: string } | undefined> {
  ...
  if ("product" in node && node.product) {
    const product = reshapeProduct(node.product, false);
    return product ? { product, variantId: node.id } : undefined;
  }

  const product = reshapeProduct(node as ShopifyProduct, false);
  return product ? { product } : undefined;
}

/** The signed-in customer's profile and their own orders, newest first. */
